...
var getEvent = async ({browser, id, wait, cookie}) => {
  ...
  var event = await page.evaluate((id) => {
    ...
    var location =
      /VarnaLab/.test(document.querySelector('[data-testid=event_permalink_feature_line]').innerText) ||
      /https:\/\/www\.facebook\.com\/varnalab/.test(document.querySelector('#event_summary').innerHTML) ||
      /50 Pencho Slaveykov str, 9000 Varna, Bulgaria/.test(document.querySelector('#event_summary').innerText)
      ? 'VarnaLab' : undefined
